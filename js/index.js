...
function draw() { 
  ...
  if (totalY<10){
    ...
    ctx.fillText(`Лыжник!`,cw/2,60);
    ctx.font = "20px Helvetica";
    ctx.fillText(`Нажимай на стрелки клавиатуры, чтобы начать`,cw/2,100);
    ctx.font = "16px Helvetica";
    ctx.fillText(`Используй ← и → для езды`,cw/2,124);
  ...
  ctx.fillText(`Счет: ${Math.floor((totalY-1)/4)} метров`,10,25);
  ...
  obstacles.forEach(function(obstacle) {
    ...
    if (obstacle.y+obstacle.height > ch/4 - 16
      ...
      ctx.fillStyle='#9B000F';
      ctx.font = "16px Helvetica";
      ctx.fillText(`ТЫ ВРЕЗАЛСЯ!`,10,60);
      ctx.fillStyle='#111213';
      ctx.fillText(`Ты проехал ${Math.floor((totalY-1)/4)} метров.`,10,80);
      ctx.fillText(`Нажми пробел, чтобы начать заново.`,10,100);
    }
  });
